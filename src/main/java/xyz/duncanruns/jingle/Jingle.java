...
public final class Jingle {
    ...
    @Nullable private static Object activeWindow = null;
    ...
    public static void start() {
        if (PlatformFactory.detectPlatform() == PlatformFactory.Platform.UNKNOWN) {
    ...
    private static synchronized void tick() {
        ...
        activeWindow = windowManager.getForegroundWindow();
    ...
    private static void updateWindowTitle(OpenedInstance instance) {
        windowManager.setWindowTitle(instance.windowHandle, "Minecraft* - Instance 1");
    }

    public static synchronized boolean isInstanceActive() {
        return getMainInstance().map(i -> Objects.equals(i.windowHandle, activeWindow)).orElse(false);
    }
    ...
    private static void updateMainInstance() {
        ...
        if (mainInstancePreviouslyExists && getMainInstance().get().windowHandle == activeWindow) return;
        ...
        Optional<OpenedInstanceInfo> newActiveInstance = allOpenedInstances.stream()
                .filter(info -> Objects.equals(activeWindow, info.windowHandle))
                .findAny();
        ...
        if (!(mainInstancePreviouslyExists && windowManager.isWindowValid(getMainInstance().get().windowHandle))) {
    ...
    private static void undoWindowTitle(OpenedInstance instance) {
        ...
        if (windowManager.isWindowValid(instance.windowHandle)) {
            windowManager.setWindowTitle(instance.windowHandle, "Minecraft*");
        }
    }
    ...
    public static synchronized void goBorderless() {
        getMainInstance().ifPresent(instance -> {
            Rectangle bounds;
            if (options.borderlessPosition == null) {
                GraphicsDevice gd = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice();
                bounds = gd.getDefaultConfiguration().getBounds();
            } else {
                bounds = new Rectangle(
                    options.borderlessPosition[0],
                    options.borderlessPosition[1],
                    options.borderlessPosition[2],
                    options.borderlessPosition[3]
                );
            }
            windowManager.setWindowBounds(instance.windowHandle, bounds);
        });
    }
